...
| [WebdriverIO Page Object pattern docs](https://webdriver.io/docs/pageobjects/) | `appium/page-objects.md` | Docs (webdriver.io) | Canonical authority behind the POM doctrine the rules enforce: selectors centralized as `get`-getters, no assertions and no per-test state in pages, pages exported as singletons, specs kept free of selectors. Cited, not copied. |
| [WebdriverIO Appium API reference](https://webdriver.io/docs/api/appium) | `appium/mobile-commands-and-context.md`, `appium/gestures-and-scrolling.md` | Docs (webdriver.io) | Authoritative source for native↔WebView context switching (`getContexts`/`switchContext`) and the deprecation of `appium*`-prefixed / `touchAction` commands in favour of the W3C mobile commands. Cited, not copied. |
