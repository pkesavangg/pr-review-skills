...
- Figma designs — target UI for BP dashboard, AHA ratings, three-reading average, multi-device snapshot cards
...
| **Reference lines** | Goal weight horizontal line | BP: systolic=120 and diastolic=80 reference lines |
...
| `ProductSelection.myBloodPressure` | Defined with display name, icon, all titles (`dashboardTitle`, `entryTitle`, `historyTitle`) | `Domain/Models/Domain/Product/ProductSelection.swift` |
...
| **Dashboard header product selector** | **Fully implemented** — navbar shows `dashboardTitle`, chevron, opens `ProductTypeSelectorSheet` when multiple products exist | `Features/Dashboard/Views/Screens/DashboardScreen.swift` (lines 124-143) |
| `ProductTypeStrings` | All needed strings defined: `myWeight`, `myBloodPressure`, `myBP`, `bpEntry`, `myDashboard`, etc. | `Features/Common/Strings/ProductTypeStrings.swift` |
...
| `AhaPressureClass` enum | Staged in git (not yet in source tree) — 5-level classification with colors | `Domain/Models/Domain/BP/AhaPressureClass.swift` |
| `BpmConstants` | Staged in git (not yet in source tree) — reference lines, validation ranges, Y-axis defaults | `Features/Dashboard/BPM/Enums/BpmConstants.swift` |
| `AhaRatingSheet` view | Staged in git (not yet in source tree) — modal with AHA classification chart | `Features/Dashboard/BPM/Views/Components/AhaRatingSheet.swift` |
| BPM folder structure | Empty directories created: `BPM/Stores/`, `BPM/Strings/`, `BPM/Views/Screens/` | `Features/Dashboard/BPM/` |
...
| **BP fields** on `BathScaleEntry` — no systolic, diastolic, meanArterial, note | Cannot store BP readings | Small |
...
| **`BpmDashboardStrings`** | BP-specific UI strings file not created (referenced by `AhaRatingSheet`) | Small |
| **`ProductTypeStrings.selectGraph`** | No "Select Graph" string (dashboard currently uses `myDashboard` — confirm with Figma if a separate string is needed) | Small |
| **Multi-device snapshot cards** | No summary card view for dashboard overview | Medium |
| **BPM Bluetooth data reception** | No handler for BP readings arriving from physical BPM device via BLE (see Section 18) | Medium |
| **BP empty state views** | No empty dashboard/graph state for zero BPM entries | Small |
| **Per-product-type metric card order persistence** | `DashboardSettings` stores a single card order; needs separate order for weight vs BPM | Small |
| **ThreeReadingAverage model** | Referenced in pseudocode but not defined as a struct | Small |
| **SelectionData enum** | Referenced in section 9.5 but not defined | Small |
| **`BpmEntryStore` DI registration** | Not yet in `ServiceRegistry` | Small |
| **HealthKit BP sync** | No HealthKit integration for BP data (see Section 19) | Medium |
| **Note field UI** | No UI for entering/viewing notes on BPM entries in manual entry or history (see Section 20) | Small |
...
│   ├── WeightTrendView → GraphView → BaseGraphView (1 weight line + optional metric)
│   └── DashboardMetricsSection (12 body metrics, goal, streaks)
└── [BPM selected] → BPM dashboard (DashboardStore with productType = .bpm)
    ├── BpmTrendView → GraphView → BaseGraphView (3 lines: systolic, diastolic, pulse)
    ├── BpmDisplayView (systolic/diastolic/pulse headline with AHA colors)
    ├── DashboardMetricsSection (AHA card, 3-reading avg, streak cards — draggable/editable)
...
| **DashboardGoalManager** | Weight goal tracking | BP mode: no goal (or future BP goal); returns nil/disabled |
...
**`DashboardScreen.swift`** — product type header selector **already implemented** (lines 124-143). Current implementation uses `ProductTypeStrings.myDashboard` as the sheet title. Confirm with Figma if this should be `"Select Graph"` instead (if so, add `selectGraph` to `ProductTypeStrings`). `ProductSelection.dashboardTitle` already returns `"My BP"` for BPM.

**Note:** `ManualEntryScreen` uses `ProductTypeStrings.manualEntry` as the sheet title, `HistoryListScreen` uses `ProductTypeStrings.myHistory`, and the dashboard uses `ProductTypeStrings.myDashboard`.
...
|--------|----------------|-------------|
| Dashboard | `WeightTrendView` + 12-metric grid + goal/streaks | `BpmTrendView` + AHA/3-reading avg + streaks |
...
| **Reference lines** | Goal weight horizontal line | Systolic=120, Diastolic=80 horizontal dashed lines | Conditional `RuleMark` in `BaseGraphView` |
...
| 2.11 | Disable goal for BP in `DashboardGoalManager` | `Managers/DashboardGoalManager.swift` |
...
## 18. BPM Bluetooth Data Reception Flow

> **Gap identified:** The document covers manual entry but does not address how BP readings arrive from a physical BPM device via Bluetooth.

### 18.1 Current Weight Scale BLE Flow (Reference)

```
BluetoothService discovers scale → BTScaleData received
  → ScaleService processes reading → Entry created with deviceType = "scale"
  → EntryService.saveNewEntry() → publishers fire → Dashboard updates
```

### 18.2 Required BPM BLE Flow

```
BluetoothService discovers BPM device → BPM reading received (systolic, diastolic, pulse)
  → ScaleService (or new handler) processes reading
  → Entry created with:
    - deviceType = "bpm"
    - entryType = "bpm"
    - scaleEntry.systolic, scaleEntry.diastolic, scaleEntry.meanArterial
    - scaleEntryMetric.pulse
  → EntryService.saveNewEntry() → bpmDailySummaries publisher fires → Dashboard updates
```

### 18.3 Open Questions — BLE

| Question | Suggested Resolution |
|----------|---------------------|
| Does `GGBluetoothSwiftPackage` support BPM devices? | Check package capabilities — it may only support scales. May need a new BPM BLE package or extension. |
| What BLE service/characteristic UUIDs does the BPM device advertise? | Need hardware team input. Standard BLE Blood Pressure Service UUID is `0x1810`. |
| How is BPM device discovery differentiated from scale discovery? | By BLE service UUID — BPM devices advertise Blood Pressure Service, scales advertise Weight Scale Service. |
| Should `BTScaleData` be extended or should a new `BTBpmData` struct be created? | New struct recommended — BP data shape (systolic/diastolic/pulse) is fundamentally different from scale data (weight/impedance). |
| Where does `meanArterial` come from — device or client calculation? | Client-side: `(systolic + 2 * diastolic) / 3`. Some BPM devices may provide it directly. |

### 18.4 Implementation Considerations

- **Phase 7 dependency:** BLE BPM support is blocked on hardware team confirming the BLE protocol and providing test devices.
- **For Phase 1-6 (local testing):** Manual entry is sufficient. BLE integration can follow the same `Entry` creation path once hardware support is available.
- **Device pairing flow:** A new BPM pairing flow is needed in `ScaleSetup` feature — similar to scale pairing but targeting BPM BLE service UUID.

---

## 19. HealthKit Integration for BP Data

> **Gap identified:** The app integrates with HealthKit for weight data. BP data should also sync to/from HealthKit.

### 19.1 HealthKit BP Types

| HKQuantityType | Identifier | Unit |
|----------------|------------|------|
...
**Note:** HealthKit stores systolic and diastolic as separate `HKQuantitySample` objects, correlated via an `HKCorrelation` of type `.bloodPressure`. Heart rate is a separate sample.

### 19.2 Sync Direction

| Direction | Behavior |
|-----------|----------|
| **Write to HealthKit** | When a BP entry is saved (manual or BLE), write systolic, diastolic, and pulse to HealthKit if the user has granted permission. |
| **Read from HealthKit** | Future consideration — importing BP readings from HealthKit into the app. Not required for Phase 1-6. |

### 19.3 Implementation

- Extend `HealthKitService` to request authorization for BP quantity types.
- Add `writeBPReading(systolic:diastolic:pulse:date:)` method to `HealthKitServiceProtocol`.
- Call from `EntryService.saveNewEntry()` when `entryType == .bpm` and HealthKit permission is granted.
- **Phase:** Can be added in Phase 5 (manual entry) or Phase 7 (API integration). Not a blocker for dashboard rendering.

---

## 20. Note Field UI

> **Gap identified:** The `note` field is added to `BathScaleEntry` but no UI is specified for entering or viewing notes.

### 20.1 Manual Entry

The BP manual entry form (`BpmManualEntryScreen`) should include an optional "Note" text field below the systolic/diastolic/pulse fields:
...
- **Validation:** Optional field, max 500 characters.
- **Persistence:** Stored in `BathScaleEntry.note`.

### 20.2 History

BP entries in history should show the note (if present) as a secondary line below the reading values:

```
┌─────────────────────────────────┐
│ 123/111  ❤️ 100  · Normal       │
│ "Took after morning walk"       │  ← note
│ Jun 29, 2025 8:30 AM           │
└─────────────────────────────────┘
```

### 20.3 Crosshair / Selection

When a point with a note is selected on the graph, the note is **not** shown on the graph itself (too much visual noise). Notes are viewable in history only.

---

## 21. Empty State Views

> **Gap identified:** The document doesn't specify what the BPM dashboard looks like when there are no BP entries.

### 21.1 BPM Dashboard Empty State

When the user switches to "My Blood Pressure" but has zero BPM entries:
...
- **"Add Reading" button** navigates to the Manual Entry tab with BPM product type selected.
- The empty state uses the same pattern as the existing weight empty state (if one exists) for visual consistency.
- **Metric cards section** is hidden when there are no entries (nothing to average or classify).

### 21.2 Multi-Device Snapshot Empty State

If the user has a BPM device but no BP entries, the BPM snapshot card shows:
...
## 22. Graph Line Behavior on Empty Days

> **Gap identified:** The document says "no interpolation" for BP but doesn't specify the visual behavior.

### 22.1 Line Continuity

When there are days without BP readings between data points:

- **Lines connect directly** between the two nearest data points (straight line segments, no gaps). This matches the Angular BPM app behavior and is the default `LineMark` behavior in Swift Charts.
- **No interpolation** means: no Hermite spline smoothing like the weight graph. BP lines are **straight segments** between actual readings, not smooth curves.
- **PointMarks** (dots) are rendered only on days with actual data.

### 22.2 Crosshair on Empty Days

When the user taps on an area between data points:

- The crosshair **snaps to the nearest data point** (by x-axis proximity).
- The headline values update to show that point's systolic/diastolic/pulse.
- The AHA color updates to that point's classification.
- This differs from the weight graph which interpolates a value for empty days.

---

## 23. BPM Streak Definition

> **Gap identified:** What constitutes a "streak" for BPM is not defined.

### 23.1 Definition

A BPM streak is defined as **consecutive calendar days with at least one BPM entry** (same logic as weight streaks, but filtered by `entryType == .bpm`).

| Metric | Definition |
|--------|-----------|
| **Current streak** | Number of consecutive days ending today (or yesterday if no reading today yet) with at least one BP reading |
| **Longest streak** | Maximum consecutive-day count across all time |

### 23.2 Data Source

Uses the same `DashboardStreakManager` as weight, with `entryType` parameter:
```swift
let streak = try await entryService.getStreak(entryType: .bpm)
```

---

## 24. Per-Product-Type Metric Card Order Persistence

> **Gap identified:** `DashboardSettings` stores a single card order. Weight and BPM have different cards.

### 24.1 Problem

The current `DashboardSettings` model stores metric card order/visibility as a single configuration. With BPM, the user has a different set of cards (AHA rating, 3-reading average, streaks) that need their own order preferences.

### 24.2 Solution

Extend `DashboardSettings` (or `DashboardSyncCoordinator`) to store card order keyed by product type:

```swift
// DashboardSettings — ADD
var bpmCardOrder: [String]?        // Card IDs for BPM metric cards
var bpmHiddenCards: [String]?      // Hidden card IDs for BPM
```

Existing `cardOrder` / `hiddenCards` fields continue to serve weight metrics. The `DashboardGridEditingManager` reads/writes the correct field set based on the active `productType`.

### 24.3 API Sync

When the backend is ready, the dashboard config sync endpoint should include product-type-keyed card preferences. For now, stored locally via SwiftData.

---

## 25. Unit Handling

> **Clarification:** BP measurement units do not vary like weight units.

- **Blood pressure** is always displayed in **mmHg** (millimeters of mercury). There is no unit conversion (unlike weight, which supports kg/lbs/st).
- **Pulse** is always displayed in **bpm** (beats per minute).
- The `Entry` model does not need a unit field for BP entries.
- The `BpmDisplayView` headline labels are hardcoded: "mmhg" and "pulse" (per Figma).
- `YAxisCalculator` does not need unit-aware formatting for BP — just integer values.

---

## 26. Accessibility

> **Gap identified:** No accessibility considerations for the BPM dashboard.

### 26.1 AHA Color Accessibility

AHA colors (green/yellow/orange/red) must be distinguishable by colorblind users:

- All AHA-colored elements must include a **text label** alongside the color (e.g., "Normal", "Elevated", "High Stage 1", "High Stage 2", "Hypertensive Crisis").
- VoiceOver should read: "Blood pressure 123 over 111, AHA classification: Normal" rather than relying on color alone.
- Consider adding a **pattern or icon** alongside the color for the graph lines (e.g., dashed for elevated, dotted for high) — but only if Figma specifies this.

### 26.2 Three-Line Graph

- VoiceOver `accessibilityLabel` for the graph: "Blood pressure chart showing systolic, diastolic, and pulse trends for [period]."
- Selected point: "Selected June 29. Systolic 123, Diastolic 111, Pulse 100. AHA classification: Normal."
- Reference lines: "Normal systolic reference at 120. Normal diastolic reference at 80."

### 26.3 Metric Cards

- `ThreeReadingAverageCard` should read: "Three reading average. Systolic 113, Diastolic 74, Pulse 63."
- `AhaRatingCard` should read: "AHA rating: Normal. Tap for details."

---

## 27. Concurrent Snapshot Data Loading

> **Gap identified:** How the multi-device snapshot view loads data for both products simultaneously.

### 27.1 Strategy

When `DashboardScreen` shows the `MultiDeviceSnapshotView`, it needs lightweight data for both weight and BPM mini graphs:

```swift
// DashboardScreen.swift — on appear for snapshot view
Task {
    async let weightLoad = entryService.loadDashboardData(entryType: .wg)
    async let bpmLoad = entryService.loadDashboardData(entryType: .bpm)
    await (weightLoad, bpmLoad) // parallel loading
}
```

- **Weight snapshot** reads from `entryService.dailySummaries` (last 7 days).
- **BPM snapshot** reads from `entryService.bpmDailySummaries` (last 7 days).
- Both publishers already exist (or will after Phase 1) — the snapshot cards simply observe them.
- **Loading state:** Each card shows its own skeleton/loading state independently.

---

## 28. Multi-Device Snapshot Navigation

> **Gap identified:** How navigation works when tapping a snapshot card.

### 28.1 Navigation Pattern

The snapshot → product dashboard transition is a **state toggle**, not a navigation push:
...
### 28.2 Back Button Behavior

- When navigating from snapshot to a product dashboard, the navbar shows a **"< back"** button (left side).
- Tapping back sets `isInProductDashboard = false`, returning to the snapshot view.
- The header dropdown is still accessible — user can switch products without going back to snapshot first.
- **Tab switching:** If the user switches tabs and returns to Dashboard, the `isInProductDashboard` state is preserved (they return to wherever they were).

---

## 29. Missing Model Definitions

> **Gap identified:** Several models are referenced in pseudocode but not formally defined.

### 29.1 ThreeReadingAverage

```swift
// Domain/Models/Domain/BP/ThreeReadingAverage.swift
struct ThreeReadingAverage {
    let systolic: Int
    let diastolic: Int
    let pulse: Int
    let count: Int                      // 1, 2, or 3
    let label: String                   // "three entry average", "two entry average", "last reading"
    let classification: AhaPressureClass
    let readings: [Entry]               // The actual entries used for the average
}
```
...
```swift
// Features/Dashboard/Models/SelectionData.swift
enum SelectionData {
...
### 29.3 BpmEntryStore DI Registration

`BpmEntryStore` needs to be registered in `ServiceRegistry` as a session-scoped service (registered after login, alongside `EntryStore`):

```swift
// ServiceRegistry.swift — session registration
let bpmEntryStore = BpmEntryStore()
DependencyContainer.shared.register(bpmEntryStore)
DependencyContainer.shared.register(bpmEntryStore as BpmEntryStoreProtocol)
```
