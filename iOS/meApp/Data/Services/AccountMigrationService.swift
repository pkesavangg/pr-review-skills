...
// MARK: - MigrationIntegrationStoreProtocol

/// Sync-only subset of integration storage used during migration.
/// (`IntegrationRepositoryProtocol` declares `async throws`; the concrete implementation
///  is synchronous, matching AccountMigrationService's non-async call sites.)
@MainActor
protocol MigrationIntegrationStoreProtocol {
    func setIntegrationData(accountId: String, info: IntegrationInfo?) throws
}

extension IntegrationRepository: MigrationIntegrationStoreProtocol {}

// MARK: - AccountMigrationService
...
final class AccountMigrationService { // swiftlint:disable:this type_body_length
    ...
    func migrateHealthKitIntegrationData(for accountId: String) { // swiftlint:disable:this function_body_length
        ...
        if isIntegrated || assignedTo != nil || deIntegrated != nil {
            ...
            do {
                try integrationStore.setIntegrationData(accountId: assignedTo ?? accountId, info: integrationInfo)
    ...
    private func findAllAccountIdsInUserDefaults() -> Set<String> {
        ...
        let allKeys = kvStorage.getAllKeys()
