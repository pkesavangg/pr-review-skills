...
@MainActor
final class DeviceService: ObservableObject, @preconcurrency PairedDeviceServiceProtocol { // swiftlint:disable:this type_body_length
    static let shared = DeviceService()
    private let tag = "DeviceService"

    @MainActor
    private lazy var remoteRepo: DeviceRepositoryAPIProtocol = // Ensure this is always created on the main actor
        _apiRepository

    private let _apiRepository: DeviceRepositoryAPIProtocol
    private let localRepository: DeviceRepositoryProtocol
    private let localKVRepo: DeviceRepositoryLocal
    private let accountService: AccountServiceProtocol
    private let logger = LoggerService.shared
    private let migrationService = DeviceMigrationService()
    ...
    init() {
        ...
        _apiRepository = DeviceAPIRepository()
        localRepository = DeviceRepository()
        localKVRepo = DeviceRepositoryLocal()
        Task {
    ...
    init(accountService: AccountServiceProtocol,
         apiRepository: DeviceRepositoryAPIProtocol? = nil,
         localRepository: DeviceRepositoryProtocol? = nil,
         localKVRepo: DeviceRepositoryLocal = DeviceRepositoryLocal()) {
        self.accountService = accountService
        _apiRepository = apiRepository ?? DeviceAPIRepository()
        self.localRepository = localRepository ?? DeviceRepository()
    ...
    func updateScaleMeta(_ deviceId: String, metaData: DeviceMetaData) async throws {
        guard try await localRepository.getDevice(deviceId) != nil else {
            throw DeviceError.deviceNotFound(id: deviceId)
        }
        ...
        if let tempDevice = tempDevice {
            ...
            if existingDevice == nil {
                do {
                    ...
                    throw DeviceError.apiSyncFailed(error)
                }
            }
        }
    ...
    private func scaleLogDescriptor(_ device: Device) -> String {
        ...
        return DeviceLogDescriptor.describe(device: device, preference: preference)
    }
    ...
    private func isDuplicateDevice(device: Device, remoteDTO: DeviceDTO) -> Bool {
    ...
    private func reconcileServerDevices(_ serverScales: [DeviceDTO], accountId: String) async throws -> Int {
        var corrected = 0
    ...
    private func shouldAttemptServerScaleMatch(_ dto: DeviceDTO, accountId: String) async -> Bool {
    ...
    private func findMatchingLocalDevice(for dto: DeviceDTO, accountId: String) async throws -> Device? {
    ...
    private func normalizeMatchedDevice(_ device: Device, with dto: DeviceDTO, accountId: String) async throws -> Int {
        var corrected = 0
    ...
    private func pruneOrphanSyncedDevices(accountId: String, serverScales: [DeviceDTO]) async throws -> Int {
    ...
    private func makeServerIdentifiers(from serverScales: [DeviceDTO]) -> DeviceServerIdentifiers {
        var ids = Set<String>()
        var macs = Set<String>()
        ...
        return DeviceServerIdentifiers(ids: ids, macs: macs, broadcastIds: broadcastIds)
    }

    private func shouldPruneDevice(_ device: Device, accountId: String, identifiers: DeviceServerIdentifiers) -> Bool {
    ...
    private func createPropertiesFromDTO(_ dto: DeviceDTO) -> [String: Any] {
    ...
    func createBluetoothScale(
        ...
        case "A3": scaleType = DeviceSourceType.bluetooth.rawValue
        case "A6": scaleType = DeviceSourceType.lcbt.rawValue
        case "R4": scaleType = DeviceSourceType.btWifiR4.rawValue
        default:   scaleType = DeviceSourceType.bluetooth.rawValue
        }
    ...
    func createA6Scale(
        ...
        if device.bathScale == nil {
            let bathScale = BathScale(
                scaleType: DeviceSourceType.lcbt.rawValue,
                bodyComp: device.bathScale?.bodyComp ?? false
            )
            device.bathScale = bathScale
        } else {
            // Update scale type if bath scale exists
            device.bathScale?.scaleType = DeviceSourceType.lcbt.rawValue
        }
        // Use the repository to create the scale with proper relationship handling
        let savedDevice = try await localRepository.createScale(device)
