...
final class DeviceRepository: DeviceRepositoryProtocol {
    ...
    func updateDeviceWithNewId(oldId: String, updatedDevice: Device) async throws {
        ...
        try context.save()
        logger.log(level: .debug, tag: "DeviceRepository", message: "Successfully updated device with new ID: \(updatedDevice.id)")
    }
    ...
    func createScale(_ scale: Device) async throws -> Device {
        ...
        try context.save()
        logger.log(level: .info, tag: "DeviceRepository", message: "Successfully created scale with ID: \(scale.id)")
        return scale
    }
    ...
    func replaceAllDevicesForAccount( // swiftlint:disable:this function_body_length
        ...
        // DeviceService.ephemeralState and merged into DeviceSnapshot during refresh.
        ...
        logger.log(level: .debug, tag: "DeviceRepository", message: "Deleting \(syncedDevices.count) synced devices for account \(accountId)")
        for device in syncedDevices {
            logger.log(level: .debug, tag: "DeviceRepository", message: "Deleting synced device: \(device.id), sku: \(device.sku ?? "nil")")
        ...
        for serverDevice in serverDevices {
            ...
            logger.log(level: .debug, tag: "DeviceRepository", message: "Inserting server device: \(serverDevice.id ?? "nil"), sku: \(serverDevice.sku ?? "nil")")
    ...
    private func fetchDeviceOrThrow(_ deviceId: String) throws -> Device {
        ...
        guard let device = try context.fetch(descriptor).first else {
            throw NSError(domain: "DeviceService", code: 404, userInfo: [NSLocalizedDescriptionKey: "Device not found"])
        }
        return device
    }
    ...
    /// Copy device fields from a DeviceDTO to a Device.
    /// Connection status (isConnected, isWifiConfigured) is intentionally not written here —
    /// it is managed via DeviceService.ephemeralState and never persisted in SwiftData.
    private func copyDeviceFields(from dto: DeviceDTO, to device: Device, accountId: String) {
    ...
    private func updateMetaData(for device: Device, from metaDataDTO: DeviceMetaDataDTO) {
        if let existing = device.metaData {
    ...
    private func copyMetaDataFields(from src: DeviceMetaDataDTO, to dst: DeviceMetaData) {
    ...
    private func updateBathDeviceModelType(for device: Device, scaleType: String?) {
    ...
    private func findMatchingUnsyncedDevice(for serverDevice: DeviceDTO, in unsyncedDevices: [Device], accountId: String) -> Device? {
    ...
    /// Updates a device from a DeviceDTO, preserving relationships.
    private func updateDeviceFromDTO(_ device: Device, from dto: DeviceDTO, accountId: String) {
        ...
        // Update bath scale type if needed
        updateBathDeviceModelType(for: device, scaleType: dto.type)
    }
    ...
    private func fetchAttachedPreferenceInternal(by id: String) -> R4ScalePreference? {
        ...
        do {
            ...
            logger.log(level: .error, tag: "DeviceRepository", message: "Failed to fetch attached R4ScalePreference: \(error.localizedDescription)")
            return nil
        }
    }
}
