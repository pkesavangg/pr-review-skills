...
struct DeviceModesScreen: View {
    ...
    // MARK: - Initializer
    init(scale: Device, isR4DeviceSetup: Bool = false, isWeighOnlyModeEnabledByOthers: Bool = false, isPresentedAsSheet: Bool = false) {
        self.scale = scale
        self.isR4DeviceSetup = isR4DeviceSetup
        self.isPresentedAsSheet = isPresentedAsSheet
        _viewModel = StateObject(wrappedValue: DeviceModesViewModel(scale: scale, isWeighOnlyModeEnabledByOthers: isWeighOnlyModeEnabledByOthers))
    }
    ...
    var body: some View {
        VStack(alignment: .center, spacing: 0) {
            NavbarHeaderView(
                title: isR4DeviceSetup ? lang.r4scaleSetupTitle : lang.modeTitle,
                leadingContent: {
                ...
                onLeadingTap: {
                    Task {
                        ...
                        let allow = isR4DeviceSetup ? true : await viewModel.allowExit()
                        if allow {
                            if isPresentedAsSheet { dismiss() } else { router.navigateBack() }
                        }
                    }
            ...
            DeviceModesSelectionView(
                ...
                isR4DeviceSetup: isR4DeviceSetup,
                onBIAButtonTap: {
        ...
        .onAppear {
            ...
            guard !isR4DeviceSetup else { return }
...
final class DeviceModesViewModel: ObservableObject {
    ...
    private var originalModeValue: DeviceModes = .weightOnly
    ...
    private let tag = "DeviceModesViewModel"
    ...
    func updateModeValue(_ mode: DeviceModes) {
        modeValue = mode
...
// swiftlint:disable:next function_body_length
    private func performSaveOperation(onSuccess: (() -> Void)? = nil) async {
        ...
        do {
            ...
            notificationService.dismissLoader()
            notificationService.showToast(ToastModel(title: ToastStrings.success, message: DeviceModesStrings.preferencesSaved))
            onSuccess?()
    ...
    private func updateDisplayMetricsForHeartRate(dto: inout R4ScalePreferenceDTO) {
        let heartRateKey = DeviceMetrics.config.first { $0.name == "Heart Rate" }?.key ?? "heartRate"
        let goalMetricsKeys = DeviceMetrics.progressMetrics.map { $0.key }
