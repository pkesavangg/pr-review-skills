...
struct DeviceTypeHelper {
    ...
    // - Returns: The determined scale type as a DeviceModelType enum
// swiftlint:disable:next cyclomatic_complexity
    static func determineDeviceModelType(for scale: Device) -> DeviceModelType {
        ...
        if let scaleSourceType = scale.bathScale?.scaleType {
            let sourceType = DeviceSourceType(rawValue: scaleSourceType) ?? .bluetoothScale
            switch sourceType {
    ...
    static func determineDeviceModelType(sku: String?, scaleType: String?, deviceType: String?) -> DeviceModelType {
        guard let sku else {
            if let scaleType {
                let sourceType = DeviceSourceType(rawValue: scaleType) ?? .bluetoothScale
                switch sourceType {
                case .bluetooth, .bluetoothScale, .lcbt, .lcbtScale:
                    return .bluetoothA6
                case .wifi, .espTouchWifi:
...
// swiftlint:disable:next cyclomatic_complexity
    static func determineDeviceModelTypeString(for scale: Device) -> String {
