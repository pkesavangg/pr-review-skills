...
struct MyDevicesScreen: View {
    @Environment(\.appTheme) private var theme
    @Environment(\.scenePhase) private var scenePhase
    @EnvironmentObject var router: Router<SettingsRoute>
    @StateObject private var scaleStore = DeviceStore()
    ...
    private enum ActiveSheet: Identifiable, Equatable {
        ...
        // Custom Equatable conformance is required because `DeviceItemInfo`
    ...
    private func determineDeviceModelType(for scale: DeviceSnapshot) -> DeviceModelType {
        return DeviceTypeHelper.determineDeviceModelType(sku: scale.sku, scaleType: scale.bathScale?.scaleType, deviceType: scale.deviceType)
    }
                ...
                VStack(alignment: .center, spacing: 0) {
                    ...
                    ButtonView(
                        ...
                        isDisabled: !scaleStore.addScaleForm.isValid
                            ...
                            let scaleWithOriginalSku = DeviceItemInfo(
                ...
                if !scaleStore.scales.isEmpty {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        ForEach(scaleStore.scales, id: \.id) { scale in
                            let scaleType = determineDeviceModelType(for: scale)
                            DeviceItemView(
                                ...
                                onTap: {
                                    if scaleType == .bpm {
                                        ...
                                        router.navigate(to: .deviceSettings(scale: scale.toDevice(), scaleType: scaleType))
                                    }
                                },
                                scaleType: scaleType
                            )
