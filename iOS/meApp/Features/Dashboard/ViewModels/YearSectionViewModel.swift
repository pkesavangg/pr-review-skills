...
class YearSectionViewModel: ObservableObject {
    ...
    /// X-axis values with buffer for year period
    var xAxisValues: [Date] {
        guard let store = dashboardStore else { return [] }
        return store.xAxisValuesWithBuffer(for: .year)
    }
    ...
    func updateYAxisConfiguration() {
        ...
        // Use visible operations for Y-axis calculation in year view (different from Total)
    ...
    /// Calculates goal chip position for year view (with X-axis adjustment)
    func getGoalChipPosition() -> (yPosition: CGFloat, placement: GoalPlacement) {
        ...
        // Account for X-axis height for year period (18px adjustment)
    ...
    func getChartPosition(for date: Date, value: Double) -> CGPoint? {
        ...
        // For year period, use scroll-based calculation
    ...
    /// Formats X-axis label for year view (month names)
    func formatXAxisLabel(for date: Date) -> String? {
        return dashboardStore?.xLabelString(for: date, period: .year)
    }
    ...
    func getConnectedSegments(from dataPoints: [GraphSeries]) -> [[GraphSeries]] {
        ...
        let maxGap: TimeInterval = 365 * 24 * 60 * 60 // 1 year gap for year view
    ...
    /// Returns appropriate point size for year view
    func getPointSizeForYear() -> CGFloat {
        return 64 // Larger points for year view (monthly data points)
    }
    ...
    /// Initialize chart for year section
