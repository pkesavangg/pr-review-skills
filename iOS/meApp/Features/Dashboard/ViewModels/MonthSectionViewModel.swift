...
@MainActor
class MonthSectionViewModel: ObservableObject {
    ...
    /// X-axis values with buffer for month period
    var xAxisValues: [Date] {
        guard let store = dashboardStore else { return [] }
        return store.xAxisValuesWithBuffer(for: .month)
    }
    ...
    func updateYAxisConfiguration() {
        ...
        // Use visible operations for Y-axis calculation in month view (different from Total)
    ...
    /// Calculates goal chip position for month view (with X-axis adjustment)
    func getGoalChipPosition() -> (yPosition: CGFloat, placement: GoalPlacement) {
        ...
        // Account for X-axis height for month period (18px adjustment)
    ...
    func getChartPosition(for date: Date, value: Double) -> CGPoint? {
        ...
        // For month period, use scroll-based calculation
    ...
    /// Formats X-axis label for month view (day names/numbers)
    func formatXAxisLabel(for date: Date) -> String? {
        return dashboardStore?.xLabelString(for: date, period: .month)
    }
    ...
    func getConnectedSegments(from dataPoints: [GraphSeries]) -> [[GraphSeries]] {
        ...
        let maxGap: TimeInterval = 60 * 24 * 60 * 60 // 60 days gap for month view (matches original month behavior)
    ...
    /// Returns appropriate point size for month view
    func getPointSizeForMonth() -> CGFloat {
        return 64 // Larger points for month view (daily data points)
    }
    ...
    /// Initialize chart for month section
