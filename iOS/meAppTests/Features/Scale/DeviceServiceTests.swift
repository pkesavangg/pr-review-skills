...
struct DeviceServiceTests {
    ...
    ) -> (DeviceService, MockScaleRepository, MockScaleRepositoryAPI, MockAccountService) {
    ...
    func deleteDeviceRemovesMacSiblings() async throws {
        let repo = MockScaleRepository()
        let a = ScaleTestFixtures.makeDevice(id: "dev-a", hasServerID: false)
        let b = ScaleTestFixtures.makeDevice(id: "dev-b", hasServerID: false)
        // a and b share MAC + userNumber (both "0") → both are deletion candidates.
        repo.devices = [a, b]
        let (sut, _, _, _) = makeSUT(repo: repo)
    ...
    func deleteDeviceKeepsDifferentUserNumber() async throws {
        ...
        let a = ScaleTestFixtures.makeDevice(id: "dev-a", hasServerID: false)
        let b = ScaleTestFixtures.makeDevice(id: "dev-b", hasServerID: false)
        b.userNumber = "1" // Different user slot on the same physical device.
        repo.devices = [a, b]
        let (sut, _, _, _) = makeSUT(repo: repo)
