//
//  ScaleRepositoryLocalTests.swift
//  meAppTests
//
...
@Suite(.serialized)
@MainActor
struct ScaleRepositoryLocalTests {

    private let expectedKeyPrefix = "scale_last_sync_timestamp"

    // MARK: - makeSUT

    private func makeSUT(kv: KvStorageServiceProtocol? = nil) -> (sut: ScaleRepositoryLocal, kv: MockKvStorageService) {
        let kvStore = kv as? MockKvStorageService ?? MockKvStorageService()
        let sut = ScaleRepositoryLocal(kv: kvStore)
        return (sut, kvStore)
    }
    ...
    @Test("setLastSyncTimestamp repeated set overwrites (non-destructive)")
    ...
    @Test("clearLastSyncTimestamp removes value get returns nil")
    ...
    @Test("clearLastSyncTimestamp repeated clear is safe (idempotent)")
    ...
    @Test("timestamp payload handling long string consistent")
