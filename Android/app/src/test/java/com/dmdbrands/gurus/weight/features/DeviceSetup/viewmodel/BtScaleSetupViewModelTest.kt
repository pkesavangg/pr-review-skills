...
class BtScaleSetupViewModelTest {
    ...
    fun `SetSku updates sku in state`() {
        viewModel.handleIntent(DeviceSetupIntent.SetSku("0999"))
    ...
    fun `handleButtonChanges disables back for SETUP_FINISHED step`() {
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(BtScaleSetupStep.SETUP_FINISHED))
        advanceScheduler()
        advanceScheduler()
        assertThat(viewModel.state.value.backEnabled).isFalse()
    }
    ...
    fun `handleButtonChanges disables back for SET_DEVICE_USER step`() {
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(BtScaleSetupStep.SET_DEVICE_USER))
        advanceScheduler()
        advanceScheduler()
        assertThat(viewModel.state.value.backEnabled).isFalse()
    }
    ...
    fun `handleButtonChanges disables next for PAIRING_MODE step`() {
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(BtScaleSetupStep.PAIRING_MODE))
        advanceScheduler()
        advanceScheduler()
        assertThat(viewModel.state.value.nextEnabled).isFalse()
    }
    ...
    fun `handleButtonChanges disables next for STEP_ON step`() {
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(BtScaleSetupStep.STEP_ON))
        advanceScheduler()
        advanceScheduler()
        assertThat(viewModel.state.value.nextEnabled).isFalse()
    }
    ...
    fun `handleButtonChanges disables next for SELECT_USER when user is null`() {
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(BtScaleSetupStep.SELECT_USER))
        advanceScheduler()
        advanceScheduler()
        assertThat(viewModel.state.value.nextEnabled).isFalse()
    }
    ...
    fun `RequestPermission calls dialogUtility permissionAlert`() {
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.RequestPermission("bluetooth"))
        advanceScheduler()
        verify {
