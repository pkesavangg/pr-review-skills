...
class BabyScaleSetupReducerTest {
    ...
    // -------------------------------------------------------------------------
    // Step navigation (inherited from DeviceSetupReducer)
    ...
    fun `SetNewStep changes to BABY_PROFILE_FORM`() {
        val result = reducer.reduce(initialState, DeviceSetupIntent.SetNewStep(BabyScaleSetupStep.BABY_PROFILE_FORM))
        assertThat(result?.step).isEqualTo(BabyScaleSetupStep.BABY_PROFILE_FORM)
    }

    @Test
    fun `SetNewStep changes to BABY_LIST`() {
        val result = reducer.reduce(initialState, DeviceSetupIntent.SetNewStep(BabyScaleSetupStep.BABY_LIST))
    ...
    fun `SetNewStep preserves nickname and babyProfiles`() {
        ...
        val result = reducer.reduce(state, DeviceSetupIntent.SetNewStep(BabyScaleSetupStep.SETUP_FINISHED))
        assertThat(result?.nickname).isEqualTo("My Scale")
        assertThat(result?.babyProfiles).hasSize(1)
    }
}
