 ...
 * delegates everything else to [DeviceSetupReducer]. Both paths are tested here.
 */
class BtScaleSetupReducerTest {
    ...
    fun `AlterConnectionState Failed_Error delegates to base reducer`() {
        ...
        val result = reducer.reduce(
            state, DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.Error)
        )
    ...
    fun `SetNewStep navigates to PERMISSIONS step`() {
        ...
        val result = reducer.reduce(state, DeviceSetupIntent.SetNewStep(BtScaleSetupStep.PERMISSIONS))
    ...
    fun `SetNewStep preserves user field when navigating`() {
        ...
        val result = reducer.reduce(state, DeviceSetupIntent.SetNewStep(BtScaleSetupStep.PAIRING_MODE))
    ...
    fun `ExitSetup returns state unchanged — handled by ViewModel`() {
        val state = defaultState()
        val result = reducer.reduce(
            state, DeviceSetupIntent.ExitSetup(isSetupFinished = false)
        )
