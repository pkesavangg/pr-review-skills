...
class MonitorSetupReducerTest {
  ...
  fun `SetNewStep changes step via base reducer`() {
    val result = reducer.reduce(initialState, ScaleSetupIntent.SetNewStep(MonitorSetupStep.PERMISSIONS))
    assertThat(result?.step).isEqualTo(MonitorSetupStep.PERMISSIONS)
  }

  @Test
  fun `BackEnabled updates backEnabled flag`() {
    val result = reducer.reduce(initialState, ScaleSetupIntent.BackEnabled(true))
    assertThat(result?.backEnabled).isTrue()
  }

  @Test
  fun `NextEnabled updates nextEnabled flag`() {
    val result = reducer.reduce(initialState, ScaleSetupIntent.NextEnabled(false))
    assertThat(result?.nextEnabled).isFalse()
  }

  @Test
  fun `AlterConnectionState updates connection state`() {
    val result = reducer.reduce(initialState, ScaleSetupIntent.AlterConnectionState(ConnectionState.Success))
  ...
  fun `SetPermissions updates permissions map`() {
    ...
    val result = reducer.reduce(initialState, ScaleSetupIntent.SetPermissions(perms))
    assertThat(result?.permissions).isEqualTo(perms)
  }

  @Test
  fun `SetSku updates sku in state`() {
    val result = reducer.reduce(initialState, ScaleSetupIntent.SetSku("0604"))
