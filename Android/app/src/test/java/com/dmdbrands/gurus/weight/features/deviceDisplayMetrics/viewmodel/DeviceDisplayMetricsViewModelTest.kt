...
class DeviceDisplayMetricsViewModelTest {
    ...
    private lateinit var viewModel: DeviceDisplayMetricsViewModel
    ...
    private fun createViewModel(): DeviceDisplayMetricsViewModel =
        DeviceDisplayMetricsViewModel(
    ...
    fun `SetScale updates scale and enabledMetrics`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(DeviceDisplayMetricsIntent.SetScale(device))
        advanceUntilIdle()
    ...
    fun `UpdateMetrics updates enabledMetrics and sets hasUpdated when changed`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(DeviceDisplayMetricsIntent.UpdateMetrics(listOf("weight", "bodyFat")))
        advanceUntilIdle()
    ...
    fun `UpdateMetrics with same metrics as original sets hasUpdated false`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(DeviceDisplayMetricsIntent.UpdateMetrics(listOf("weight")))
        advanceUntilIdle()
    ...
    fun `Save with null scale shows error toast`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(DeviceDisplayMetricsIntent.Save)
        advanceUntilIdle()
    ...
    @Test
    fun `UpdateScaleMode navigates to DeviceMode route`() = runTest(mainDispatcherRule.scheduler) {
        viewModel = createViewModel()
        advanceUntilIdle()

        viewModel.handleIntent(DeviceDisplayMetricsIntent.UpdateScaleMode)
        advanceUntilIdle()

        coVerify { navigationService.navigateTo(AppRoute.DeviceDetails.DeviceMode(TEST_SCALE_ID)) }
    }
}
