...
class AddDeviceViewModelTest {
    ...
    private fun createViewModel(): AddDeviceViewModel =
        AddDeviceViewModel(
    ...
    fun `SetSavedScales updates savedScales in state`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(AddDeviceIntent.SetSavedScales(devices))
        advanceUntilIdle()
    ...
    fun `OpenSelectedScaleSetup updates selectedSku`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(AddDeviceIntent.OpenSelectedScaleSetup(TEST_SKU))
        advanceUntilIdle()
    ...
    @Test
    fun `OpenDeviceSettings updates scaleId`() = runTest(mainDispatcherRule.scheduler) {
        viewModel = createViewModel()
        advanceUntilIdle()

        viewModel.handleIntent(AddDeviceIntent.OpenDeviceSettings(TEST_SCALE_ID))
        advanceUntilIdle()
    ...
    fun `Submit sets isSubmitting to true`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(AddDeviceIntent.Submit)
        advanceUntilIdle()
    ...
    @Test
    fun `OpenScaleChooser navigates to ChooseDevice route`() = runTest(mainDispatcherRule.scheduler) {
        viewModel = createViewModel()
        advanceUntilIdle()

        viewModel.handleIntent(AddDeviceIntent.OpenScaleChooser)
        advanceUntilIdle()
    ...
    @Test
    fun `OpenDeviceSettings navigates to DeviceDetails route`() = runTest(mainDispatcherRule.scheduler) {
        viewModel = createViewModel()
        advanceUntilIdle()
    ...
    fun `ShowHelp calls dialogUtility showModelNumberHelpDialog`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(AddDeviceIntent.ShowHelp)
        advanceUntilIdle()
    ...
    fun `ResetForm resets model number form value`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(AddDeviceIntent.ResetForm)
        advanceUntilIdle()
    ...
    fun `OpenSelectedScaleSetup with unknown SKU does not navigate`() = runTest(mainDispatcherRule.scheduler) {
        ...
        viewModel.handleIntent(AddDeviceIntent.OpenSelectedScaleSetup("9999"))
        advanceUntilIdle()
