...
/**
 * Unit tests for [ScaleModeReducer].
 *
 * ScaleModeReducer is a pure function — no mocking or coroutines needed.
 * Each test creates an initial state, dispatches an intent, and asserts the result.
 */
class ScaleModeReducerTest {

    private lateinit var reducer: ScaleModeReducer

    private fun defaultState() = ScaleModeState()
    ...
    @Test
    fun `default ScaleModeState has expected initial values`() {
    ...
    fun `SetScale stores provided device`() {
        ...
        val result = reducer.reduce(defaultState(), ScaleModeIntent.SetScale(device))
    ...
    fun `SetScale replaces previous device`() {
        ...
        val result = reducer.reduce(state, ScaleModeIntent.SetScale(deviceB))
    ...
    fun `SetScale preserves mode flags`() {
        ...
        val result = reducer.reduce(state, ScaleModeIntent.SetScale(device))
    ...
    fun `SetMode updates isAllBodyMetrics and hasModeChanged`() {
        ...
        val result = reducer.reduce(
            state,
            ScaleModeIntent.SetMode(isAllBodyMetrics = false, hasModeChanged = true),
        )
    ...
    fun `SetMode with isAllBodyMetrics true and hasModeChanged false reflects mode unchanged`() {
        ...
        val result = reducer.reduce(
            state,
            ScaleModeIntent.SetMode(isAllBodyMetrics = true, hasModeChanged = false),
        )
    ...
    fun `SetHeartRate enables heart rate and marks mode changed`() {
        ...
        val result = reducer.reduce(
            state,
            ScaleModeIntent.SetHeartRate(isHeartRateOn = true, hasModeChanged = true),
        )
    ...
    fun `SetHeartRate disables heart rate`() {
        ...
        val result = reducer.reduce(
            state,
            ScaleModeIntent.SetHeartRate(isHeartRateOn = false, hasModeChanged = false),
        )
    ...
    fun `Save returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleModeIntent.Save)
    ...
    fun `Back returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleModeIntent.Back)
    ...
    fun `OpenBiaModal returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleModeIntent.OpenBiaModal)
