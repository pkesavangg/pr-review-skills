...
class LcbtBLESetupViewModelTest {
    ...
    fun `SetSku updates sku in state`() {
        createViewModel()
        viewModel.handleIntent(DeviceSetupIntent.SetSku("1234"))
    ...
    fun `AlterConnectionState updates connection state to Failed Error`() {
        createViewModel()
        viewModel.handleIntent(DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.Error))
        assertThat(viewModel.state.value.scaleSetupState.setupState.connectionState)
    ...
    fun `AlterConnectionState updates to Failed ErrorWithMessage`() {
        ...
        viewModel.handleIntent(
            DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.ErrorWithMessage("BT_001"))
        )
    ...
    fun `SetPermissions updates permissions in state`() {
        ...
        viewModel.handleIntent(DeviceSetupIntent.SetPermissions(permissions))
    ...
    fun `SetNewStep changes current step`() {
        createViewModel()
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.PERMISSIONS))
    ...
    fun `SetNewStep to SETUP_FINISHED sets isLastStep true`() {
        createViewModel()
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.SETUP_FINISHED))
    ...
    fun `Next from intermediate step advances to next step`() {
        createViewModel()
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.WAKEUP))
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.Next)
        advanceScheduler()
    ...
    fun `TryAgain on CONNECTING_BLUETOOTH sets connection state to Loading`() {
        createViewModel()
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.CONNECTING_BLUETOOTH))
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.Error))
        advanceScheduler()
        viewModel.handleIntent(DeviceSetupIntent.TryAgain)
    ...
    fun `connectToBluetooth aborts auto-advance when state flips to Failed during success hold`() {
        ...
        viewModel.handleIntent(
            DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.Error)
        )
