...
/**
 * Unit tests for [ScaleDisplayMetricsReducer].
 *
 * The reducer is a pure function — no coroutines or complex mocks needed.
 * Key logic under test:
 *  - [ScaleDisplayMetricsIntent.SetScale] populates enabledMetrics from device preferences
 *  - [ScaleDisplayMetricsIntent.UpdateMetrics] tracks whether changes differ from the saved metrics
 */
class ScaleDisplayMetricsReducerTest {

    private lateinit var reducer: ScaleDisplayMetricsReducer
    ...
    private fun defaultState() = ScaleDisplayMetricsState()
    ...
    @Test
    fun `default ScaleDisplayMetricsState has expected initial values`() {
    ...
    fun `SetScale stores device and populates enabledMetrics from preferences`() {
        ...
        val result = reducer.reduce(defaultState(), ScaleDisplayMetricsIntent.SetScale(device))
    ...
    fun `SetScale resets hasUpdated to false`() {
        ...
        val result = reducer.reduce(state, ScaleDisplayMetricsIntent.SetScale(device))
    ...
    fun `SetScale replaces previously loaded device`() {
        ...
        val state = reducer.reduce(defaultState(), ScaleDisplayMetricsIntent.SetScale(deviceA))!!

        val result = reducer.reduce(state, ScaleDisplayMetricsIntent.SetScale(deviceB))
    ...
    fun `UpdateMetrics stores new metrics list`() {
        val device = deviceWithMetrics(INITIAL_METRICS)
        val state = reducer.reduce(defaultState(), ScaleDisplayMetricsIntent.SetScale(device))!!

        val newMetrics = listOf(METRIC_WEIGHT, METRIC_BMI, METRIC_BODY_FAT)
        val result = reducer.reduce(state, ScaleDisplayMetricsIntent.UpdateMetrics(newMetrics))
    ...
    fun `UpdateMetrics sets hasUpdated to false when metrics match saved preferences`() {
        ...
        val result = reducer.reduce(
            state, ScaleDisplayMetricsIntent.UpdateMetrics(INITIAL_METRICS)
        )
    ...
    fun `UpdateMetrics with empty list when original was non-empty marks hasUpdated`() {
        ...
        val result = reducer.reduce(state, ScaleDisplayMetricsIntent.UpdateMetrics(emptyList()))
    ...
    fun `UpdateMetrics when scale has null preferences treats original as empty`() {
        ...
        val result = reducer.reduce(
            state, ScaleDisplayMetricsIntent.UpdateMetrics(listOf(METRIC_WEIGHT))
        )
    ...
    fun `UpdateMetrics preserves the loaded scale reference`() {
        ...
        val result = reducer.reduce(
            state, ScaleDisplayMetricsIntent.UpdateMetrics(listOf(METRIC_BODY_FAT))
        )
    ...
    fun `Save sets hasUpdated to false`() {
        ...
        val result = reducer.reduce(modifiedState, ScaleDisplayMetricsIntent.Save)
    ...
    fun `Back returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleDisplayMetricsIntent.Back)
    ...
    fun `UpdateScaleMode returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleDisplayMetricsIntent.UpdateScaleMode)
