...
/**
 * Unit tests for [ScaleSetupReducer] — the base reducer shared across all scale setup flows.
 *
 * Because ScaleSetupReducer is open and parameterised, these tests use the concrete
...
class ScaleSetupReducerTest {
    ...
    // A default initial state backed by BtScaleSetupState (uses base ScaleSetupState internally)
    ...
    fun `AlterConnectionState to Loading changes connectionState`() {
        ...
        val result = reducer.reduce(state, ScaleSetupIntent.AlterConnectionState(ConnectionState.Loading))
    ...
    fun `AlterConnectionState to Failed_Error changes connectionState`() {
        ...
        val result = reducer.reduce(state, ScaleSetupIntent.AlterConnectionState(ConnectionState.Failed.Error))
    ...
    fun `SetSku preserves other base state fields`() {
        ...
        val stateWithPerms = reducer.reduce(state, ScaleSetupIntent.SetPermissions(perms))!!

        val result = reducer.reduce(stateWithPerms, ScaleSetupIntent.SetSku(TEST_SKU))
    ...
    @Test
    fun `SetScaleInfo stores provided ScaleInfo`() {
        val state = defaultState()
        val info = ScaleInfo(
            productName = "Smart Scale",
            sku = TEST_SKU,
            setupType = com.dmdbrands.gurus.weight.features.common.enums.ScaleSetupType.Bluetooth,
            bodyComp = true,
        )

        val result = reducer.reduce(state, ScaleSetupIntent.SetScaleInfo(info))
    ...
    fun `SetNewStep to a step already in list updates current step`() {
        ...
        val result = reducer.reduce(
            state, ScaleSetupIntent.SetNewStep(BtScaleSetupStep.PAIRING_MODE)
        )
    ...
    fun `SetNewStep to a new step appends it to the steps list`() {
        ...
        val result = reducer.reduce(
            state, ScaleSetupIntent.SetNewStep(BtScaleSetupStep.SETUP_FINISHED)
        )
    ...
    fun `SetNewStep with wrong Step type returns state with base updated`() {
        ...
        val result = reducer.reduce(
            state,
            ScaleSetupIntent.SetNewStep(com.dmdbrands.gurus.weight.features.ScaleSetup.enums.LcbtScaleSetupStep.WAKEUP)
        )
    ...
    fun `Skip intent is not handled by base reducer — returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleSetupIntent.Skip)
    ...
    fun `ExitSetup intent is not handled by base reducer — returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleSetupIntent.ExitSetup(isSetupFinished = true))
    ...
    fun `RequestPermission intent is not handled by base reducer — returns state unchanged`() {
        ...
        val result = reducer.reduce(state, ScaleSetupIntent.RequestPermission(PERMISSION_BLUETOOTH))
    ...
    fun `isLastStep is true when on SETUP_FINISHED`() {
        val state = defaultState()
        val result = reducer.reduce(state, ScaleSetupIntent.SetNewStep(BtScaleSetupStep.SETUP_FINISHED))!!
