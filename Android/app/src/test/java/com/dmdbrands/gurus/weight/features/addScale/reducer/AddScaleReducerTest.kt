...
/**
 * Unit tests for [AddScaleReducer].
 *
 * AddScaleReducer is a pure function — no mocking or coroutines needed.
 ...
 */
class AddScaleReducerTest {

    private lateinit var reducer: AddScaleReducer
    ...
    fun `Submit sets isSubmitting to true`() {
        ...
        val result = reducer.reduce(state, AddScaleIntent.Submit)
    ...
    fun `OpenSelectedScaleSetup stores sku`() {
        ...
        val result = reducer.reduce(state, AddScaleIntent.OpenSelectedScaleSetup(TEST_SKU))
    ...
    @Test
    fun `OpenScaleSettings stores scaleId`() {
        val result = reducer.reduce(
            defaultState(), AddScaleIntent.OpenScaleSettings(TEST_SCALE_ID)
        )
    ...
    @Test
    fun `OpenScaleSettings replaces previous scaleId`() {
        val state = defaultState().copy(scaleId = "old-scale-id")

        val result = reducer.reduce(state, AddScaleIntent.OpenScaleSettings(TEST_SCALE_ID))
    ...
    fun `SetSavedScales with empty list stores empty savedScales`() {
        val result = reducer.reduce(defaultState(), AddScaleIntent.SetSavedScales(emptyList()))
    ...
    fun `SetSavedScales with single device stores its scaleInfo`() {
        ...
        val result = reducer.reduce(defaultState(), AddScaleIntent.SetSavedScales(listOf(device)))

        // One device → one ScaleInfo entry
        assertThat(result?.savedScales).hasSize(1)
    }

    @Test
    fun `SetSavedScales with multiple devices stores all as ScaleInfo and sorts by createdAt descending`() {
        ...
        val result = reducer.reduce(
            defaultState(),
            AddScaleIntent.SetSavedScales(listOf(deviceA, deviceB)),
        )
    ...
    fun `SetSavedScales replaces previous savedScales`() {
        val deviceA: Device = mockk(relaxed = true)
        val state = reducer.reduce(defaultState(), AddScaleIntent.SetSavedScales(listOf(deviceA)))!!

        val result = reducer.reduce(state, AddScaleIntent.SetSavedScales(emptyList()))
    ...
    fun `ShowHelp returns state unchanged`() {
        ...
        val result = reducer.reduce(state, AddScaleIntent.ShowHelp)
    ...
    fun `OpenScaleChooser returns state unchanged`() {
        ...
        val result = reducer.reduce(state, AddScaleIntent.OpenScaleChooser)
    ...
    fun `ResetForm returns state unchanged by reducer`() {
        ...
        val result = reducer.reduce(state, AddScaleIntent.ResetForm)
