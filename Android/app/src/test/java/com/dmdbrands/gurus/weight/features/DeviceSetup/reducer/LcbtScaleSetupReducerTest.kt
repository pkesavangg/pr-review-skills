 ...
 *
 * LcbtScaleSetupReducer has no own intents; it delegates everything to [DeviceSetupReducer].
...
class LcbtScaleSetupReducerTest {
    ...
    fun `AlterConnectionState to Success changes connectionState`() {
        val result = reducer.reduce(
            defaultState(), DeviceSetupIntent.AlterConnectionState(ConnectionState.Success)
        )
    ...
    fun `AlterConnectionState to Failed_Error changes connectionState`() {
        val result = reducer.reduce(
            defaultState(), DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.Error)
        )
    ...
    fun `AlterConnectionState to Loading changes connectionState`() {
        ...
        val result = reducer.reduce(
            state, DeviceSetupIntent.AlterConnectionState(ConnectionState.Loading)
        )
    ...
    fun `SetPermissions updates permissions map`() {
        ...
        val result = reducer.reduce(defaultState(), DeviceSetupIntent.SetPermissions(perms))
    ...
    fun `SetPermissions with empty map clears existing permissions`() {
        val perms: GGPermissionStatusMap = mutableMapOf(PERMISSION_BLUETOOTH to "true")
        val state = reducer.reduce(defaultState(), DeviceSetupIntent.SetPermissions(perms))!!
    ...
    fun `SetSku updates sku field`() {
        val result = reducer.reduce(defaultState(), DeviceSetupIntent.SetSku(TEST_SKU))
    ...
    fun `SetScaleInfo stores scale info`() {
        val info = DeviceModelInfo(
            ...
            sku = TEST_SKU,
            setupType = com.dmdbrands.gurus.weight.features.common.enums.DeviceSetupType.Lcbt,
            bodyComp = true,
        )

        val result = reducer.reduce(defaultState(), DeviceSetupIntent.SetScaleInfo(info))
    ...
    fun `BackEnabled true enables back`() {
        val result = reducer.reduce(defaultState(), DeviceSetupIntent.BackEnabled(true))
    ...
    fun `NextEnabled false disables next`() {
        val result = reducer.reduce(defaultState(), DeviceSetupIntent.NextEnabled(false))
    ...
    fun `SetNewStep navigates to WAKEUP step`() {
        val result = reducer.reduce(
            defaultState(), DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.WAKEUP)
        )
    ...
    fun `SetNewStep navigates to CONNECTING_BLUETOOTH step`() {
        val result = reducer.reduce(
            defaultState(), DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.CONNECTING_BLUETOOTH)
        )
    ...
    fun `SetNewStep navigates to SETUP_FINISHED step and isLastStep becomes true`() {
        val result = reducer.reduce(
            defaultState(), DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.SETUP_FINISHED)
        )
    ...
    fun `ExitSetup intent returns state unchanged — handled by ViewModel`() {
        val state = defaultState()
        val result = reducer.reduce(
            state, DeviceSetupIntent.ExitSetup(isSetupFinished = true)
        )
