...
class SignupViewModel @Inject constructor(
     ...
     */
    fun onNext() {
        AppLog.d("SignupViewModel", "onNext called - currentStep: ${state.value.currentStep}, isLastStep: ${state.value.isLastStep}, isValid: ${state.value.isCurrentStepValid}")
        ...
        if (state.value.isLastStep) {
            ...
            handleIntent(SignupIntent.Next)
     ...
     */
    fun onBack() {
        handleIntent(SignupIntent.Back)
    }
    ...
    /**
     * Handles moving to a specific step.
     */
    fun goToStep(step: SignupStep) {
        handleIntent(SignupIntent.GoToStep(step))
    }
    ...
    fun onSubmit() {
        ...
        val isFormValid = if (state.value.goalSkipped) {
            // When goal is skipped, validate all fields except goal-related ones
            val controls = state.value.form.controls
            val basicFieldsValid = listOf(
                controls.firstName.validate(),
                controls.lastName.validate(),
                controls.email.validate(),
                controls.password.validate(),
        ...
        viewModelScope.launch {
            try {
                ...
                val signupRequest = mutableMapOf<String, Any>(
                    "email" to controls.email.value.trim(),
                    "firstName" to controls.firstName.value.trim(),
                    "lastName" to controls.lastName.value.trim().ifEmpty { " " },
                    "gender" to controls.sex.value,
                    "zipcode" to controls.zipcode.value.trim().ifEmpty { " " },
                ...
                if (!state.value.goalSkipped) {
                    ...
                    val convertedCurrentWeight = ConversionTools.convertDisplayToStored(
                        display = currentWeight,
                        isMetric = isMetric
                    )
                    val convertedGoalWeight = ConversionTools.convertDisplayToStored(
                        display = goalWeight,
                        isMetric = isMetric
                    )

                    goalData = if (goalType == "maintain") {
                        ...
                        val determinedGoalType = if (convertedGoalWeight > convertedCurrentWeight) "gain" else "lose"
                        mapOf(
                ...
                if (account != null) {
                    ...
                    if (!state.value.goalSkipped && goalData != null) {
                        try {
                            ...
                            AppLog.w("SignupViewModel", "Goal/bodycomp update failed but continuing", goalException.toString())
                        }
                    }
    ...
    private fun convertHeightInputToMm(heightInput: HeightInput): Int {
        return when (heightInput) {
            ...
            is HeightInput.FtIn -> {
                ...
                val storedHeight = ConversionTools.convertFeetInchesToStoredHeight(
