...
class LcbtScaleSetupViewModel
  ...
  constructor(
  ...
  ) : BaseIntentViewModel<LcbtScaleSetupState, LcbtScaleSetupIntent>(
      reducer = LcbtScaleSetupReducer(),
    ) {
    @AssistedFactory
    interface Factory {
      fun create(sku: String): LcbtScaleSetupViewModel
    }
    ...
    private fun loadScaleInfo() {
      ...
      handleIntent(LcbtScaleSetupIntent.SetScaleSku(sku))
    }
