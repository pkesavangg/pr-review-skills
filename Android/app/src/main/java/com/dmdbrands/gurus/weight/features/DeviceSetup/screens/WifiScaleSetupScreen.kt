...
import com.dmdbrands.gurus.weight.features.DeviceSetup.components.WifiItem
...
import com.dmdbrands.gurus.weight.features.DeviceSetup.viewmodel.WifiScaleSetupViewModel
...
fun WifiScaleSetupScreenContent(
  ...
  DeviceSetupHeader(
    ...
    HorizontalPagerWithBottomNavigation(
      ...
      leadingContent = { WifiLeadingButton(state = state, onIntent = onIntent) },
      middleContent = { WifiMiddleButton(state = state, onIntent = onIntent) },
      trailingContent = {
        WifiTrailingContent(state = state, focusManager = focusManager, onIntent = onIntent)
      },
      pageContent = { WifiPageContent(state = state, onIntent = onIntent) },
    )
  }
}

@Composable
private fun WifiLeadingButton(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  AppButton(
...
@Composable
private fun WifiTrailingContent(
  state: WifiScaleSetupState,
  ...
  when (state.currentStep) {
    ...
    WifiScaleSetupStep.TROUBLE_SHOOTING ->
      WifiDefaultButton(state = state, focusManager = focusManager, onIntent = onIntent)
  }
}

// Default button content for all steps
@Composable
private fun WifiDefaultButton(
  state: WifiScaleSetupState,
  focusManager: FocusManager,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
...
@Composable
private fun WifiPageContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  Crossfade(targetState = state.currentStep) { step ->
    when (step) {
      WifiScaleSetupStep.SCALE_INFO -> WifiScaleInfoContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.PERMISSIONS -> WifiPermissionsContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.WIFI_PASSWORD -> WifiPasswordContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.SELECT_USER -> WifiSelectUserContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.ACTIVATE_SCALE -> WifiActivateScaleContent(state = state)
      WifiScaleSetupStep.WIFI_MODE -> WifiModeContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.ERROR_GUIDE -> WifiErrorGuideContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.TROUBLE_SHOOTING -> WifiTroubleShootingContent()
      WifiScaleSetupStep.ERROR_CODE_SELECTED -> WifiErrorCodeSelectedContent(state = state)
      WifiScaleSetupStep.SWITCH_WIFI -> WifiSwitchWifiContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.SCALE_COUNTS -> WifiScaleCountsContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.MAC_ADDRESS -> WifiMacAddressContent(state = state, onIntent = onIntent)
      WifiScaleSetupStep.STEP_ON -> WifiStepOnContent()
      WifiScaleSetupStep.SETUP_FINISHED -> WifiSetupFinishedContent()
    }
  }
}

@Composable
private fun WifiScaleInfoContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  com.dmdbrands.gurus.weight.features.DeviceSetup.components.DeviceInfoContent(
    sku = state.sku,
    setupType = DeviceSetupType.Wifi,
    buttonText = DeviceSetupStrings.DeviceInfo.WifiScaleButtonText,
    onButtonClick = { onIntent(WifiScaleSetupIntent.OnGetScaleMacAddress()) },
  )
}

@Composable
private fun WifiPermissionsContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  DevicePermissions(
...
@Composable
private fun WifiPasswordContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  SetupForm(
...
@Composable
private fun WifiSelectUserContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
...
@Composable
private fun WifiActivateScaleContent(state: WifiScaleSetupState) {
  SetupContent(
...
private fun wifiModeButtons(state: WifiScaleSetupState): List<SelectButtonItem> =
  when {
...
private fun WifiModeContent(
  ...
  val wifiButtons = wifiModeButtons(state)
  SelectButton(
    title =  when {
      state.isGetMACSetup || state.permissionsSkipped -> {
...
@Composable
private fun WifiErrorGuideContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  ...
  SelectButton(
    ...
    supportingButtonLabel = DeviceSetupStrings.SetupButtons.SomethingElse,
    onSupportingButtonClick = {
...
@Composable
private fun WifiTroubleShootingContent() {
  SetupContent(
...
@Composable
private fun WifiErrorCodeSelectedContent(state: WifiScaleSetupState) {
  ErrorContent(
...
@Composable
private fun WifiSwitchWifiContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  SetupContent(
...
@Composable
private fun WifiScaleCountsContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
  SetupContent(
    title = WifiScaleSetupStrings.DeviceCount.Title,
    isGifImage = false,
...
@Composable
private fun WifiMacAddressContent(
  state: WifiScaleSetupState,
  onIntent: (WifiScaleSetupIntent) -> Unit,
) {
...
@Composable
private fun WifiSetupFinishedContent() {
  SetupContent(
