package com.dmdbrands.gurus.weight.features.DeviceMetricsSetting.components
...
private fun DisplayMetricItemPreview() {
  MeAppTheme {
    Column {
      ...
      // Dragging metric
      DeviceMetricItem(
        metric = deviceMetrics.first().copy(isIncluded = false),
        isDragging = true,
        onToggle = {},
      )
      Spacer(Modifier.height(spacing.md))
      ...
      DeviceMetricItem(
        metric = deviceMetrics.first().copy(isEnabled = false),
        isDragging = false,
        onToggle = {},
      )
    }
