...
import com.dmdbrands.gurus.weight.features.DeviceSetup.reducer.LcbtScaleSetupReducer
  ...
  override fun onNext() {
    ...
    if (currentState.isLastStep) {
      ...
      if (isPermissionGranted) {
        handleIntent(DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.WAKEUP))
      } else {
        // Check and request permissions sequentially
        handleIntent(DeviceSetupIntent.SetNewStep(LcbtScaleSetupStep.PERMISSIONS))
        permissionAccess()
      }
    } else {
  ...
  private fun connectToBluetooth() {
    ...
    viewModelScope.launch {
      try {
        ...
        clearBluetoothTimeout()
        handleIntent(DeviceSetupIntent.AlterConnectionState(ConnectionState.Failed.ErrorWithMessage("BT_002")))
      }
    }
  }
      ...
      try {
        subscribePermissions().collect { newPermissions: GGPermissionStatusMap ->
          val areRequiredPermissionsEnabled =
            AppPermissionsHelper.areRequiredPermissionsEnabled(newPermissions, setupType = DeviceSetupType.Lcbt)
