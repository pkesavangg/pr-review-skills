...
fun DashboardMetrics(
  ...
  val (visibleMetrics, hiddenMetrics) = rememberVisibleAndHiddenMetrics(
...
/**
 * Derives the visible and hidden metric lists for the given inputs, caching each
 * intermediate calculation with [remember] to avoid recomputation.
 */
@Composable
private fun rememberVisibleAndHiddenMetrics(
  metricData: List<PeriodBodyScaleSummary>,
  ...
  val visibleMetrics = remember(dashboardMetric, metricKeys, isFromSetup, dashboardType) {
    val metrics = StatHelper.getMetrics(
      dashboardMetric,
    ...
    filterMetricsByType(metrics, dashboardType)
  }
  ...
  return visibleMetrics to hiddenMetrics
}

/**
 * Filters the supplied metrics down to those allowed for the given dashboard type.
 */
private fun filterMetricsByType(
  metrics: List<Stat>,
  dashboardType: DashboardType,
): List<Stat> {
  return when (dashboardType) {
    ...
    DashboardType.DASHBOARD_12_METRICS -> metrics
  }
}
...
private fun DashboardMetricsGrid(
  ...
  val columnCount = when (getDeviceType()) {
    DeviceType.Tablet -> 4
...
/**
 * Visible (reorderable) metric cards for the dashboard grid.
 */
private fun LazyGridScope.visibleMetricItems(
  visibleMetrics: List<Stat>,
...
/**
 * Hidden metric cards, shown only when in edit mode.
 */
private fun LazyGridScope.hiddenMetricItems(
  hiddenMetrics: List<Stat>,
  inEditMode: Boolean,
  isFromSetup: Boolean,
  onMetricMoved: (fromVisible: Boolean, toVisible: Boolean, metric: Stat) -> Unit,
