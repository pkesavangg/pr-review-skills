...
fun AppFab(
  ...
  val position = remember { Animatable(Offset(offsetX.value, offsetY.value), Offset.VectorConverter) }
  ...
  val bottomPadding = maxOf(systemBarsPadding.calculateBottomPadding(), imePadding.calculateBottomPadding())
  ...
  FloatingActionButton(
    onClick = onClick,
    shape = CircleShape,
    containerColor = MeTheme.colorScheme.primaryAction, // Solid blue color to match design
    modifier = Modifier
    ...
    Icon(
      ...
      modifier = Modifier
        .size(32.dp)
        .background(color = Color.Transparent), // Larger icon size to match design
    )
  }
}
