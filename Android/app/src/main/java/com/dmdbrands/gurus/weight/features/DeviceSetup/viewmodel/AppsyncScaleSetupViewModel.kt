...
import com.dmdbrands.gurus.weight.features.DeviceSetup.reducer.AppsyncScaleSetupReducer
      ...
      try {
        permissionService.permissionCallBackFlow.collect { permissions ->
          val areRequiredPermissionsEnabled =
            AppPermissionsHelper.areRequiredPermissionsEnabled(permissions, setupType = DeviceSetupType.AppSync)
    ...
    when (currentState.currentStep) {
      ...
      AppsyncScaleSetupStep.PERMISSIONS -> {
        val areRequiredPermissionsEnabled = AppPermissionsHelper
          .areRequiredPermissionsEnabled(state.value.permissions, setupType = DeviceSetupType.AppSync)
        val currentState = state.value
    ...
    if (isSetupFinished && state.value.isLastStep) {
      ...
      dialogQueueService.enqueue(
        DialogModel.Confirm(
          ...
          message = DeviceSetupStrings.ExitSetupAlert.Message(false),
  ...
  private fun checkAndSaveScale() {
    ...
    viewModelScope.launch {
      try {
        ...
        val productName = scaleInfo?.productName ?: DeviceSetupStrings.UnknownScale
        ...
        val appSyncDevice = Device(
          ...
          sku = currentSku,
          deviceType = DeviceSetupType.AppSync.value,
    ...
    val listWithScale = withTimeoutOrNull(DeviceSetupConstants.WAIT_FOR_SCALE_IN_LIST_MS) {
      deviceService.pairedScales.first { list ->
        list.any { device ->
          ...
          else device.sku == currentSku && device.deviceType == DeviceSetupType.AppSync.value
        }
      }
    }
