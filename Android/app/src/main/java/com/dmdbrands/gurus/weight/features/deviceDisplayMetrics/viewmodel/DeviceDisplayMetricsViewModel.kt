package com.dmdbrands.gurus.weight.features.deviceDisplayMetrics.viewmodel
...
import com.dmdbrands.gurus.weight.features.deviceDisplayMetrics.reducer.DeviceDisplayMetricsReducer
...
@HiltViewModel(
  assistedFactory = DeviceDisplayMetricsViewModel.Factory::class,
)
class DeviceDisplayMetricsViewModel
@AssistedInject
constructor(
  private val deviceService: IDeviceService,
  private val ggDeviceService: GGDeviceService,
  @Assisted val scaleId: String,
) : BaseIntentViewModel<DeviceDisplayMetricsState, DeviceDisplayMetricsIntent>(
  reducer = DeviceDisplayMetricsReducer(),
) {
  @AssistedFactory
  interface Factory {
    fun create(scaleId: String): DeviceDisplayMetricsViewModel
  }
  ...
  override fun provideInitialState(): DeviceDisplayMetricsState = DeviceDisplayMetricsState()

  override fun handleIntent(intent: DeviceDisplayMetricsIntent) {
    super.handleIntent(intent)
    when (intent) {
      DeviceDisplayMetricsIntent.Back -> onBack()
      DeviceDisplayMetricsIntent.Save -> saveDisplayMetrics()
      DeviceDisplayMetricsIntent.UpdateScaleMode -> onUpdateScaleMode()
      else -> {}
    }
  }
    ...
    viewModelScope.launch {
      deviceService.pairedScales.collect { devices ->
        ...
        device?.let { scaleDevice ->
          handleIntent(DeviceDisplayMetricsIntent.SetScale(scaleDevice))
        }
      }
    }
  }
  ...
  private fun saveDisplayMetrics() {
    ...
    if (scale == null) {
      showToast(DeviceDisplayMetricsStrings.Toast.Error)
      return
    }
    viewModelScope.launch {
      try {
        dialogQueueService.showLoader(message = DeviceDisplayMetricsStrings.LoaderMessage)
        ...
        if (success) {
          ...
          delay(1000)
          showToast(DeviceDisplayMetricsStrings.Toast.Success)
          navigateBack()
        } else {
          showToast(DeviceDisplayMetricsStrings.Toast.Error)
          dialogQueueService.dismissLoader()
        ...
        AppLog.e("DeviceDisplayMetricsViewModel", "Failed to save display metrics", err)
  ...
  private fun updateAccountFailedAlert(onRetry: () -> Unit) {
    dialogQueueService.enqueue(
      DialogModel.Confirm(
        title = DeviceDisplayMetricsStrings.UpdateAccountFailedAlert.Title,
        message = DeviceDisplayMetricsStrings.UpdateAccountFailedAlert.Message,
        confirmText = DeviceDisplayMetricsStrings.UpdateAccountFailedAlert.Retry,
        cancelText = DeviceDisplayMetricsStrings.UpdateAccountFailedAlert.Cancel,
        onConfirm = onRetry,
        onCancel = null,
      ),
    )
