    ...
    constructor(
        ...
        private fun updateStateWithAccount(currentAccount: Account) {
            ...
            val weightValidators =
                if (isWeightlessOn) {
                    listOf(
                        com.dmdbrands.gurus.weight.features.common.helper.form.FormValidations
                            .required(),
                    )
                } else {
                    emptyList()
            ...
            val newState =
                WeightlessState(
                    ...
                    hasToggleChanged = false,
                    weightUnit = if (isMetric) WeightUnit.KG else WeightUnit.LB,
                    isMetric = isMetric,
                )
        ...
        private fun onSubmit() {
            ...
            viewModelScope.launch {
                try {
                    val weightlessWeight =
                        if (isWeightlessOn) {
                            ConversionTools
                                .convertDisplayToStored(weightlessWeightDisplay.toIntOrNull() ?: 0, isMetric)
                                .toDouble()
                        } else {
                            null
                        }
