...
import com.dmdbrands.gurus.weight.features.deviceMode.reducer.DeviceModeReducer
...
@HiltViewModel(
  assistedFactory = DeviceModeViewModel.Factory::class,
)
...
constructor(
...
) : BaseIntentViewModel<DeviceModeState, DeviceModeIntent>(
  reducer = DeviceModeReducer(),
) {
  @AssistedFactory
  interface Factory {
    fun create(scaleId: String): DeviceModeViewModel
  }

  companion object {
    private const val MAX_RETRY_COUNT = 2
    private const val RETRY_DELAY_MS = 2000L // 2 seconds
    private const val TAG = "DeviceModeViewModel"
  }
   ...
   */
  private class DeviceBusyException : Exception("Scale is busy: user selection in progress")

  override fun provideInitialState(): DeviceModeState = DeviceModeState()

  override fun handleIntent(intent: DeviceModeIntent) {
    AppLog.d(TAG, "Handling scale mode intent: ${intent::class.simpleName}")
    super.handleIntent(intent)
    when (intent) {
      DeviceModeIntent.Back -> onBack()
      DeviceModeIntent.Save -> saveModeSettings()
      DeviceModeIntent.OpenBiaModal -> openBiaModel()
      else -> {}
    }
  }

  init {
    AppLog.d(TAG, "DeviceModeViewModel initialized for scaleId: $scaleId")
    initScaleMode()
  }
      ...
      try {
        deviceService.pairedScales.collect { devices ->
          ...
          device?.let { scaleDevice ->
            ...
            handleIntent(DeviceModeIntent.SetScale(scaleDevice))
            ...
            handleIntent(DeviceModeIntent.SetMode(shouldMeasureImpedance, false))
            handleIntent(DeviceModeIntent.SetHeartRate(shouldMeasurePulse, false))
          }
        }
      } catch (e: Exception) {
   ...
   * @throws DeviceBusyException if the scale is actively in a user selection session.
      ...
      ggDeviceService.updateAccount(updatedScale) { responseType ->
        when (responseType) {
          ...
          GGUserActionResponseType.USER_SELECTION_IN_PROGRESS -> {
            continuation.resumeWithException(DeviceBusyException())
          }
          else -> {
  ...
  private fun updateAccountFailedAlert(onRetry: () -> Unit) {
    dialogQueueService.enqueue(
      DialogModel.Confirm(
        title = DeviceModeStrings.UpdateAccountFailedAlert.Title,
        message = DeviceModeStrings.UpdateAccountFailedAlert.Message,
        confirmText = DeviceModeStrings.UpdateAccountFailedAlert.Retry,
        cancelText = DeviceModeStrings.UpdateAccountFailedAlert.Cancel,
        onConfirm = onRetry,
        onCancel = null,
      ),
    )
  ...
  private fun scaleBusyAlert() {
    dialogQueueService.enqueue(
      DialogModel.Alert(
        title = DeviceModeStrings.DeviceBusyAlert.Title,
        message = DeviceModeStrings.DeviceBusyAlert.Message,
  ...
  private fun saveModeSettings() {
    ...
    dialogQueueService.showLoader(
      message = DeviceModeStrings.LoaderMessage,
    )
  ...
  private fun onBack() {
    if (state.value.hasModeChanged) {
      ...
      dialogQueueService.enqueue(
        DialogModel.Confirm(
          title = DeviceModeStrings.UnsavedChanges.Title,
          message = DeviceModeStrings.UnsavedChanges.Message,
          confirmText = DeviceModeStrings.UnsavedChanges.Leave,
          cancelText = DeviceModeStrings.UnsavedChanges.Cancel,
          onConfirm = {
