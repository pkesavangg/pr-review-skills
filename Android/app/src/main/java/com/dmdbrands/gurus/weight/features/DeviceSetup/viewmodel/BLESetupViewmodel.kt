...
import com.dmdbrands.gurus.weight.features.DeviceSetup.reducer.BaseState
import com.dmdbrands.gurus.weight.features.DeviceSetup.reducer.DeviceSetupIntent
import com.dmdbrands.gurus.weight.features.DeviceSetup.reducer.SetupState
...
abstract class BLESetupViewmodel<Step : DeviceSetupStep, State : BaseState<Step, State>>(
  val protocolType: String,
  val setupInitData: SetupInitData<Step>,
  val reducer: IReducer<State, DeviceSetupIntent>,
  protected val dependencies: BLESetupDependencies,
) : BaseIntentViewModel<State, DeviceSetupIntent>(reducer) {
  ...
  abstract fun onStepChange(step: DeviceSetupStep)
  ...
  protected fun permissionAccess() {
    ...
    if (currentPermissions[GGPermissionType.BLUETOOTH_SWITCH] != GGPermissionState.ENABLED) {
      ...
      handleIntent(DeviceSetupIntent.RequestPermission(GGPermissionType.BLUETOOTH_SWITCH))
      return
    }
    ...
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
      if (currentPermissions[GGPermissionType.NEARBY_DEVICE] != GGPermissionState.ENABLED) {
        ...
        isProcessingPermissions = true
        startPermissionTimeout()
        handleIntent(DeviceSetupIntent.RequestPermission(GGPermissionType.NEARBY_DEVICE))
        return
      }
    } else {
      ...
      if (currentPermissions[GGPermissionType.LOCATION_SWITCH] != GGPermissionState.ENABLED) {
        ...
        isProcessingPermissions = true
        startPermissionTimeout()
        handleIntent(DeviceSetupIntent.RequestPermission(GGPermissionType.LOCATION_SWITCH))
        return
      }
      ...
      if (currentPermissions[GGPermissionType.LOCATION] != GGPermissionState.ENABLED) {
        ...
        isProcessingPermissions = true
        startPermissionTimeout()
        handleIntent(DeviceSetupIntent.RequestPermission(GGPermissionType.LOCATION))
        return
      }
    }
    ...
    if (isSetupFinished) {
      ...
      dialogQueueService.enqueue(
        DialogModel.Confirm(
          ...
          message = DeviceSetupStrings.ExitSetupAlert.Message(discoveredScale?.connectionStatus == BLEStatus.CONNECTED),
  ...
  private fun loadScaleInfo() {
    ...
    handleIntent(DeviceSetupIntent.SetNewStep(initialStep))
    handleIntent(DeviceSetupIntent.SetScaleInfo(scaleInfo))
    viewModelScope.launch {
      try {
        if (broadcastId != null) {
        ...
        handleIntent(DeviceSetupIntent.SetSku(sku))
      } catch (e: Exception) {
